...
{
    public class MultipleChoiceProcessor : IQuestionProcessor
    {
                                      ...
                                      ref int templateRow, ProcessingConfig config, ref int questionNumber)
        {
            ...
            {
                templateRow++;
                char optionLetter = (char)(optionNumber + 64);
                
                templateSheet.Cell(templateRow, Constants.TemplateColumn).Value = option;
                templateSheet.Cell(templateRow, Constants.CatalogueIdColumn).Value = $"{config.CatalogueIdPrefix}_{questionId}{optionLetter.ToString().ToLower()}";
                templateSheet.Cell(templateRow, Constants.QuestionIdColumn).Value = $"{config.QuestionIdPrefix}_{questionId}{optionLetter.ToString().ToLower()}";
            ...
            Logger.Log($"Multiple Choice Question {questionNumber}: {question.Text.Substring(0, Math.Min(60, question.Text.Length))}... ({question.Options.Count} options)");
